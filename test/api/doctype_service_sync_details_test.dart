...
void main() {
  ...
  List<Map<String, String>> _entries(int n) => [
        for (var i = 0; i < n; i++)
          {'doctype': 'DT$i', 'since': '2026-01-01 00:00:00.000000'},
      ];

  http.Response _manifestFor(http.Request req) {
  ...
  test('at the 100-doctype cap: exactly ONE call (legacy behaviour unchanged)',
      () async {
    final sizes = <int>[];
    final svc = _svc(MockClient((req) async {
      sizes.add(((jsonDecode(req.body)['doctypes']) as List).length);
      return _manifestFor(req);
    }));
    final r = await svc.getSyncDetails(_entries(100));
  ...
  test('over the cap: a single failed chunk => null (all-or-nothing full pull)',
      () async {
    var call = 0;
    final svc = _svc(MockClient((req) async {
      call++;
    ...
    final r = await svc.getSyncDetails(_entries(150));
    expect(r, isNull, reason: 'any chunk failure must fall back to full pull');
  });
}
