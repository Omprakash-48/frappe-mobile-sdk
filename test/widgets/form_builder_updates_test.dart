...
class _FormBuilderTestWrapperState extends State<FormBuilderTestWrapper> {
  ...
  Widget build(BuildContext context) {
    return FrappeFormBuilder(meta: _meta, initialData: _initialData);
  }
}

void main() {
  testWidgets(
    'BaseField wraps long label in Expanded and uses CrossAxisAlignment.start',
    (tester) async {
      final meta = DocTypeMeta(
        ...
        fields: [
          DocField(
            ...
            fieldtype: 'Data',
            label:
                'Very long field label that should wrap gracefully on multiple lines without throwing any RenderFlex overflow',
          ),
        ],
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: FrappeFormBuilder(meta: meta)),
        ),
      );
      ...
      final expandedTextFinder = find.byWidgetPredicate(
        (widget) =>
            widget is Expanded &&
  ...
  testWidgets(
    'FrappeFormBuilder didUpdateWidget does not recreate TabController for content-equal maps',
    (tester) async {
      final meta = DocTypeMeta(
        name: 'Customer',
  ...
  testWidgets(
    'FrappeFormBuilder didUpdateWidget recreates TabController when meta or data content changes without crashing',
    (tester) async {
      ...
      key.currentState!.updateForm(initialData: const {'name': 'Bob'});
      await tester.pump();
      ...
      key.currentState!.updateForm(meta: meta2);
      await tester.pump();
