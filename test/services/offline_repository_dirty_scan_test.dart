...
/// `sqliteTableExists` + `sqliteColumnExists` round-trips before each query.
/// The rewrite reads table existence + `sync_status` presence from a single
/// `sqlite_master` scan. These tests pin the OBSERVABLE contract (which rows
/// come back, missing-table / no-sync_status tolerance) so the perf refactor
/// cannot silently drop or mis-scan dirty docs.
...
void main() {
  ...
  setUp(() async {
    ...
    final localWriter =
        LocalWriter(appDb.rawDatabase, (dt) async => metaFor(dt));
    ...
    for (final e in metas.entries) {
      await appDb.doctypeMetaDao
          .upsertMetaJson(e.key, jsonEncode(e.value.toJson()));
    }
  ...
  test('returns dirty/deleted/error/blocked across tables, excludes synced/clean',
      () async {
    ...
    expect(
      docs.map((d) => d.data['sync_status']).toSet(),
      {'dirty', 'sync_error', 'deleted', 'sync_blocked'},
    );
  ...
  test('tolerates a doctype_meta row whose docs__ table does not exist',
      () async {
    await insertRow('Customer', 'c1', 'dirty');
    // Ghost is enrolled in doctype_meta with a table_name but no table exists.
    await appDb.doctypeMetaDao
        .upsertMetaJson('Ghost', jsonEncode(_meta('Ghost').toJson()));
  ...
  test('skips a table that has no sync_status column (child/link table)',
      () async {
    await insertRow('Customer', 'c1', 'dirty');
    ...
    await appDb.rawDatabase.insert(
      'docs__childthing',
      {'mobile_uuid': 'x1', 'title': 'child'},
    );
    await appDb.doctypeMetaDao
        .upsertMetaJson('ChildThing', jsonEncode(_meta('ChildThing').toJson()));
