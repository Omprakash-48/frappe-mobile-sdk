...
void main() {
  ...
  setUp(() async {
    ...
    for (final s in buildParentSchemaDDL(parentMeta, tableName: 'docs__order')) {
      await db.execute(s);
    }
  ...
  test('local attach values are enqueued and rewritten to pending markers',
      () async {
    ...
    final photoRow =
        pend.firstWhere((r) => r['parent_fieldname'] == 'photo');
    ...
    final receiptRow =
        pend.firstWhere((r) => r['parent_fieldname'] == 'receipt');
  ...
  test('re-saving with the same local files does not duplicate pending rows',
      () async {
    await writer.writeParent(parentDoctype: 'Order', data: saveData());
    await writer.writeParent(parentDoctype: 'Order', data: saveData());
  ...
  test('re-saving with pending markers leaves them untouched (no re-enqueue)',
      () async {
    await writer.writeParent(parentDoctype: 'Order', data: saveData());
    final parent = (await db.query('docs__order')).single;
    final child = (await db.query('docs__order_item')).single;
    ...
    expect(parent2['photo'], parent['photo']); // no 'pending:pending:' corruption
  });
}
