...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        pipeline(
          onUpload: (_) async =>
              throw ValidationException('File size exceeded the maximum', {'exc_type': 'MaxFileSizeReachedError'}),
  ...
  test('F1b: the blocked reason names the file so the user can act', () async {
    ...
    try {
      await pipeline(
        onUpload: (_) async => throw ValidationException('too big', {'exc_type': 'MaxFileSizeReachedError'}),
      ).resolveForTopParent('p-uuid-1');
